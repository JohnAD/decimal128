...
proc conversionTest(testNo: int, hexStr: string, canonicalStr: string, lossy: bool, nonCanonicalStrs: seq[string]) =
  let decoded = hexStr.decodeDecimal128(encoding=ceBID)
  let s = $decoded
  if s != canonicalStr:
    echo "testing $1: $2 and $3".format(testNo, canonicalStr, hexStr)
  ...
  for nonCanonStr in nonCanonicalStrs:
    let ncParsed = newDecimal128(nonCanonStr)
    if ncParsed !== decoded:
      echo "  parsing non-canonical string `$1` ...".format(nonCanonStr)
      echo "testing $1: $2 and $3".format(testNo, canonicalStr, hexStr)
      echo "  Error. Parsed Decimal128 did not match decoded Decimal128."
      echo "  decoded: " & decoded.repr
      echo "  parsed: " & ncParsed.repr
      raise newException(ValueError, "bad parsing")
  #
  let encoded = decoded.encodeDecimal128(encoding = ceBID)
  let encodedHex = encoded.toHex.toLower
  if encodedHex != hexStr.toLower:
    echo "  encoding original encoded back to binary..."
    echo "testing $1: $2 and $3".format(testNo, canonicalStr, hexStr)
    echo "    Error. Encoded string did not match original. Got:"
    echo "      " & encodedHex
    echo "    but expected:"
    echo "      " & hexStr
    raise newException(ValueError, "bad encoding")
...
suite "IEEE 754-2008 with BID encoding":
  test "binary-to-Decimal128-to-string and back; including non-canonical checks":
